...
public:
	...
	long GetUTM(_dPoint inPoint, _dPoint& utmPoint, long param = 0);	// 좌표 변환 (WGS84 -> UTM)
	long GetUTM2KW(long izone, _dPoint utmPoint, KW& kw); // 좌표 변환 (UTM -> WGS84)

	std::string GetGP2MGRS(_dPoint inPoint);					// 좌표 변환 (WGS84 -> MGRS)
	std::string GetUTM2MGRS(int zone, _dPoint utmPoint);
	bool GetMGRS2KW(std::string mgrs, KW& kw);				// 좌표 변환 (MGRS -> WGS84)
...
public:
	...
	long					m_CosValue[360];
	long					m_SinValue[360];
	CGeoCoordinate			m_Coordinate;				// 좌표변환 
	int						m_BaseZone;
